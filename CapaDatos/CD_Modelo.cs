    ...
    public class CD_Modelo
    {
        ...
        public int Registrar(Modelo obj, out string Mensaje)
        {
            ...
            try
            {
                using (MySqlConnection oconexion = new MySqlConnection(Conexion.cadena))
                {
                    MySqlCommand cmd = new MySqlCommand("InsertarModelo", oconexion);
        ...
        public bool Editar(Modelo obj, out string Mensaje)
        {
            ...
            try
            {
                using (MySqlConnection oconexion = new MySqlConnection(Conexion.cadena))
                {
                    MySqlCommand cmd = new MySqlCommand("EditarModelo", oconexion);
        ...
        public bool Eliminar(Modelo obj, out string Mensaje)
        {
            ...
            try
            {
                ...
                using (MySqlConnection oconexion = new MySqlConnection(Conexion.cadena))
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("EliminarModelo", oconexion);
                    cmd.Parameters.AddWithValue("p_Id", obj.Id);
                    cmd.Parameters.Add("p_Resultado", MySqlDbType.Int32).Direction = ParameterDirection.Output;
                    cmd.Parameters.Add("p_Mensaje", MySqlDbType.VarChar, 500).Direction = ParameterDirection.Output;
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public int ObtenerIdPorNombre(string nombre)
        {
            ...
            using (MySqlConnection oconexion = new MySqlConnection(Conexion.cadena))
            {
                try
                {
                    string query = "SELECT Id FROM Modelo WHERE Nombre = @Nombre";
